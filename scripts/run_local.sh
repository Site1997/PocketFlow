...
extra_args_path=`python3 utils/get_path_args.py local ${py_script} path.conf`
extra_args="${extra_args} ${extra_args_path}"
echo "python3 script: ${py_script}"
echo "# of GPUs: ${nb_gpus}"
echo "extra arguments: ${extra_args}"

# obtain list of idle GPUs
idle_gpus=`python3 utils/get_idle_gpus.py ${nb_gpus}`
export CUDA_VISIBLE_DEVICES=${idle_gpus}

# re-create the logging directory
rm -rf logs && mkdir logs

# execute the specified python3 script with one or more GPUs
